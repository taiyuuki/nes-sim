// NES 2C02 palette (64 colors, RGBA)
export const NES_PALETTE: [number, number, number, number][] = [
...
// 预计算 Uint8ClampedArray 查找表，避免每帧逐像素数组索引
const PALETTE_LUT = new Uint8ClampedArray(64 * 4)
for (let i = 0; i < 64; i++) {
    const c = NES_PALETTE[i]
    PALETTE_LUT[i * 4] = c[0]
    PALETTE_LUT[i * 4 + 1] = c[1]
    PALETTE_LUT[i * 4 + 2] = c[2]
    PALETTE_LUT[i * 4 + 3] = c[3]
}
