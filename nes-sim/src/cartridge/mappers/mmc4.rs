...
/// MMC4 board (iNES mapper 010, Fire Emblem / Famicom Wars).
///
/// Shares the MMC2 tile-$FD/$FE CHR latch, but banks 16 KiB of PRG ROM at
/// $8000 instead of 8 KiB.
pub(super) struct Mmc4 {
    prg_rom: Vec<u8>,
    prg_ram: Vec<u8>,
    chr: ChrMemory,
    prg_bank: u8,
        ...
        match (&mut self.chr, has_chr_ram) {
            ...
            _ => {
                return Err(SaveStateError::InvalidData(
                    "CHR RAM mismatch for MMC4 save state",
                ));
            }
        }
