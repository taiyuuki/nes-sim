...
/// CNE Decathlon board (iNES mapper 244). PRG and CHR banks are selected
/// by writing to specific address ranges ($8065-$80A4 for the 32 KiB PRG
/// bank, $80A5-$80E4 for the 8 KiB CHR bank).
pub(super) struct Mapper244 {
    prg_rom: Vec<u8>,
    chr: ChrMemory,
    prg_bank: usize,
    chr_bank: usize,
...
impl Mapper for Mapper244 {
    ...
    fn cpu_write(&mut self, addr: u16, _data: u8) -> bool {
        match addr {
            0x8065..=0x80A4 => {
                self.prg_bank = (addr - 0x8065) as usize & 0x3;
                true
            }
            0x80A5..=0x80E4 => {
                self.chr_bank = (addr - 0x80A5) as usize & 0x7;
                true
            }
            _ => false,
        }
