...
/// Waixing DQVII / ZS board (iNES mapper 242). The write address selects
/// the 32 KiB PRG bank and the mirroring bit; the CHR window is fixed and
/// $6000-$7FFF is WRAM.
pub(super) struct Mapper242 {
    prg_rom: Vec<u8>,
    prg_ram: Vec<u8>,
    chr: ChrMemory,
    prg_bank: usize,
...
impl Mapper for Mapper242 {
    ...
    fn cpu_write(&mut self, addr: u16, data: u8) -> bool {
        match addr {
            ...
            0x8000..=0xFFFF => {
                self.prg_bank = (addr as usize) >> 3;
                self.mirroring = if addr & 0x2 != 0 {
