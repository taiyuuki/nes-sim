...
/// "QJ" multicart board (iNES mapper 047, Super Spike V'Ball + Nintendo
/// World Cup). MMC3 core plus a single game-select bit at $6000-$7FFF that
/// selects which 128 KiB half of PRG/CHR the MMC3 bank numbers address.
pub(super) struct Mapper47 {
    prg_rom: Vec<u8>,
    chr: ChrMemory,
    mirroring: Mirroring,
    ex_reg: u8,
...
impl Mapper for Mapper47 {
    ...
    fn load_state(&mut self, reader: &mut StateReader<'_>) -> Result<(), SaveStateError> {
        ...
        self.ex_reg = reader.read_u8()? & 0x1;
        ...
        match (&mut self.chr, has_chr_ram) {
            ...
            _ => {
                return Err(SaveStateError::InvalidData(
                    "CHR RAM presence mismatch for mapper 47 save state",
                ));
            }
        }
